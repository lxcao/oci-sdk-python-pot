...
configfile = "/Users/caolingxin/.oci/config"  # Define config file to be used. 
...
config = oci.config.from_file(configfile,"specialist2-4sdk")
...
for region in regions:
  ...
  identity = oci.identity.IdentityClient(config)
  user = identity.get_user(config["user"]).data
  RootCompartmentID = "ocid1.compartment.oc1..aaaaaaaayjcsmu5ii7ac3kncp5qlbsslaj7irtc3mo4oco22w7ucsiq3atmq"
  ...
  RootCompartment.name = "gwm-compartment"
